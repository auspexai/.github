...
**Raw model outputs (opt-in capture).** If your experiment DECLARES raw-content capture (`[capture] raw`) and you collect the raw outputs (an R3-only, audited action), then **custody of and full legal responsibility for that raw model-generated content — including any sensitive, unsafe, or unlawful material it may contain — transfers to you at the moment of collection**, exactly as for computed results above. The network never stores raw outputs at rest: they transit the coordinator in memory only and are collected live by your driver during the run. Auspex is a courier for this content, not a custodian or publisher of it; handling it lawfully once collected is solely your responsibility as the data owner.
