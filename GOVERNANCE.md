...
Adverse actions — declining an application, demoting or suspending an account, quarantining a worker (including the automated quarantine on a signed containment violation), demoting a Researcher's research-standing, or revoking own-code ("BYOT") eligibility — are contestable. Every such action is taken for a **recorded reason** (§7) and may be appealed.
